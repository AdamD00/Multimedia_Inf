...
public class PlatformerMovement : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        // Reycast
        RaycastHit2D wallHit = Physics2D.BoxCast(transform.position, transform.localScale, 0f, transform.right, wallRaycasOffset, layerMask);
        if(wallHit.collider != null)
        {
            myRigidbody.velocity = new Vector2(0, myRigidbody.velocity.y);
        }

        RaycastHit2D groundHit = Physics2D.BoxCast(transform.position, transform.localScale, 0f, Vector3.down, groundRaycasOffset, layerMask);
    ...
    {
        Gizmos.DrawWireCube(transform.position + new Vector3(0,-groundRaycasOffset,0), transform.localScale);
        Gizmos.DrawWireCube(transform.position + new Vector3(wallRaycasOffset, 0, 0), transform.localScale);
    }
